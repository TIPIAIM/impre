    ...
    axios.get(`${import.meta.env.VITE_APIserveur}/recupparidFrs/${id}`)
      .then(response => {
        if (response.data) {
          ...
          navigate('/listeFournisseur'); // Redirection en cas de fournisseur non trouvé
        }
      })
    ...
    if (validate()) {
      axios.put(`${import.meta.env.VITE_APIserveur}/metajourlerecupererFrs/${id}`, values)
        .then(result => {
          ...
          navigate('/listeFournisseur'); // Redirection après une mise à jour réussie
        })
  ...
  return (
    ...
    >
      <div className="justify-center">
        <Link to="/gestionfournisseur" className="animate-pulse mx-6 my-3 bg-pink-800 text-white font-serif font-bold p-2 rounded-full">
