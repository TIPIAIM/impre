...
export default function Seconnect () {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${import.meta.env.VITE_APIserveur}/seconnecter`, { email, password }); // Envoi des données de connexion au serveur
  ...
  return (
    ...
    >
      ...
      className=' bg-black'
      >
        <h6 className=' text-blue-800 font-serif'>
          <Typewriter
            words={['Mes salutations à vous', 'Vous êtes sur la bonne page','Saisissez vos informations']}
            loop={10}
            cursor
            cursorStyle=" -"
            typeSpeed={100}
        ...
        {/*<Link to='/senregistrer' className=' no-underline font-serif' style={{ textAlign: 'center', color: '#007bff', marginTop: '10px', }}>
